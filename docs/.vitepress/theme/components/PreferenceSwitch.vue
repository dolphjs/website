<script setup lang="ts">
  ...
  const toggleSpringRouting = useToggleFn(
    preferSpringKey,
    preferSpring,
    "prefer-spring"
  );
  const toggleExpress = useToggleFn(
    preferExpressKey,
    preferExpress,
    "prefer-express"
  );
  ...
  <div v-if="show" class="preference-switch">
    ...
    <div id="preference-switches" :hidden="!isOpen" :aria-hidden="!isOpen">
      <div class="switch-container">
        <label class="options-label" @click="toggleSpringRouting(false)"
          >Express</label
        >
        <VTSwitch
          class="api-switch"
          aria-label="prefer spring routing"
          :aria-checked="preferSpring"
          @click="toggleSpringRouting()"
        />
        <label class="spring-label" @click="toggleSpringRouting(true)"
          >Spring</label
        >
        <a
          class="switch-link"
          title="About Routing preference"
      ...
      <div class="switch-container" v-if="showSFC">
        <label class="no-sfc-label" @click="toggleExpress(false)">HTML</label>
        <VTSwitch
          class="express-switch"
          aria-label="prefer express routing"
          :aria-checked="preferExpress"
          @click="toggleExpress()"
        />
        <label class="express-label" @click="toggleExpress(true)"
          >Express</label
        >
        <a
          class="switch-link"
          title="About Express"
          href="/guide/scaling-up/express.html"
          @click="closeSideBar"
          >?</a
        >
      </div>
...
<style>
  ...
  .prefer-spring .options-api,
  .prefer-sfc .html {
    display: none;
  }

  .prefer-spring .spring-api,
  .prefer-sfc .sfc {
    display: initial;
  }

  .prefer-spring .api-switch .vt-switch-check {
    transform: translateX(18px);
  }

  .spring-label,
  .express-label,
  .prefer-spring .express-label,
  .prefer-express .no-express-label {
    color: var(--vt-c-text-3);
  }

  .prefer-spring .spring-label,
  .prefer-express .express-label {
    color: var(--vt-c-text-1);
  }

  .prefer-express .express-switch .vt-switch-check {
    transform: translateX(18px);
  }
