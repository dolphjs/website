<script setup lang="ts">
  ...
  import { inBrowser, preferSpring, preferSpringKey } from "./preferences";
  ...
  let source: Source | false =
    inBrowser && localStorage.getItem(preferSpringKey) === null
      ? "default"
      : false;
  ...
  function setPreference(capi: boolean, s: Source) {
    if (capi && !preferSpring.value) {
      source = s;
      preferSpring.value = true;
      document.documentElement.classList.add("prefer-spring");
    } else if (!capi && preferSpring.value) {
      source = s;
      preferSpring.value = false;
      document.documentElement.classList.remove("prefer-spring");
    }
  }
  ...
  function dismiss() {
    ...
    // save if default
    if (source === "default") {
      localStorage.setItem(preferSpringKey, String(preferSpring.value));
    }
  }
</script>
  ...
  <Transition name="fly-in">
    <div class="preference-tooltip" v-if="show">
      <template v-if="source === 'default'">
        <p>Routing style now defaults to Spring routing.</p>
        <p>
          Some pages contain different content based on the routing style
          chosen. Use this switch to toggle between routing styles.
        </p>
      </template>
      <template v-if="source && source.startsWith('url')">
        <p>
          Showing content for
          {{ preferSpring ? "Spring" : "Express" }} routing because
          {{
            source === "url-query"
              ? "it is specified by the URL query."
              : "the target section is only available for that routing."
          }}
        </p>
        <p>
          This is different from your saved preference and will only affect the
